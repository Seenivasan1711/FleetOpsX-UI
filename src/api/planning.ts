...
export const generatePlanOptions = (plan_date: string, user_hints?: string): Promise<PlanOptionsApiResponse> =>
  client.post('/api/v1/plan/options', null, {
    params: { plan_date, ...(user_hints?.trim() ? { user_hints: user_hints.trim() } : {}) },
  }).then(r => r.data)
